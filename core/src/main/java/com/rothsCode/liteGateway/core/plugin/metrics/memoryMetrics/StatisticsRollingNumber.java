package com.rothsCode.liteGateway.core.plugin.metrics.memoryMetrics;
...
/**
 * * A number which can be used to track counters (increment) or set values over time. * <p> * It is
 * "rolling" in the sense that a 'timeInMilliseconds' is given that you want to track (such as 10
 * seconds) and then that is broken into buckets (defaults to 10) so that the 10 second window *
 * doesn't empty out and restart every 10 seconds, but instead every 1 second you have a new bucket
 * added and one dropped so that 9 of the buckets remain and only the newest starts from scratch. *
 * <p> * This is done so that the statistics are gathered over a rolling 10 second window with data
 * being added/dropped in 1 second intervals (or whatever granularity is defined by the arguments)
 * rather * than each 10 second window starting at 0 again. * <p> * Performance-wise this class is
 * optimized for writes, not reads. This is done because it expects far higher write volume
 * (thousands/second) than reads (a few per second). * <p> * For example, on each read to
 * getSum/getCount it will iterate buckets to sum the data so that on writes we don't need to
 * maintain the overall sum and pay the synchronization cost at each write to * ensure the sum is
 * up-to-date when the read can easily iterate each bucket to get the sum when it needs it. * <p> *
 * See UnitTest for usage and expected behavior examples. * * @ThreadSafe 参考 HystrixRollingNumber
 *
 * @author roths
 * @Description: 环形数组
 * @date 2023/8/29 14:11
 */
public class StatisticsRollingNumber {
  ...
  /**
   * Construct a counter, with configurable properties for how many buckets, and how long of an
   * interval to track
   *
   * @param timeInMilliseconds length of time to report metrics over
   * @param numberOfBuckets    number of buckets to use
   * @deprecated Please use {@link StatisticsRollingNumber(int, int) instead}.  These values are no
   * longer allowed to be updated at runtime.
   */

  public StatisticsRollingNumber(int timeInMilliseconds, int numberOfBuckets) {
    this(null, ACTUAL_TIME, timeInMilliseconds, numberOfBuckets);
  }

  public StatisticsRollingNumber(String resourceValue, int timeInMilliseconds,
      int numberOfBuckets) {
    this(resourceValue, ACTUAL_TIME, timeInMilliseconds, numberOfBuckets);
  }

  StatisticsRollingNumber(String resourceValue, Time time, int timeInMilliseconds,
      int numberOfBuckets) {
  ...
  /**
   * Increment the counter in the current bucket by one for the given {@link StatisticsTypeEnum}
   * type.
   * <p>
   * The {@link StatisticsTypeEnum} must be a "counter" type <code>StatisticsTypeEnum.isCounter() ==
   * true</code>.
   *
   * @param type StatisticsTypeEnum defining which counter to increment
   */
  public void increment(StatisticsTypeEnum type) {
    getCurrentBucket().getAdder(type).increment();
  }

  /**
   * Add to the counter in the current bucket for the given {@link StatisticsTypeEnum} type.
   * <p>
   * The {@link StatisticsTypeEnum} must be a "counter" type <code>StatisticsTypeEnum.isCounter() ==
   * true</code>.
   *
   * @param type  StatisticsTypeEnum defining which counter to add to
   * @param value long value to be added to the current bucket
   */
  public void add(StatisticsTypeEnum type, long value) {
   ...
   * <p>
   * The {@link StatisticsTypeEnum} must be a "max updater" type <code>StatisticsTypeEnum.isMaxUpdater()
   * == true</code>.
   *
   * @param type  StatisticsTypeEnum defining which counter to retrieve values from
   * @param value long value to be given to the max updater
   */
  public void updateRollingMax(StatisticsTypeEnum type, long value) {
   ...
   * {@link StatisticsTypeEnum} type.
   * <p>
   * See {@link #getRollingSum(StatisticsTypeEnum)} for the rolling sum.
   * <p>
   * The {@link StatisticsTypeEnum} must be a "counter" type <code>StatisticsTypeEnum.isCounter() ==
   * true</code>.
   *
   * @param type StatisticsTypeEnum defining which counter to retrieve values from
   * @return cumulative sum of all increments and adds for the given {@link StatisticsTypeEnum}
   * counter type
   */
  public long getCumulativeSum(StatisticsTypeEnum type) {
  ...
  /**
   * Get the sum of all buckets in the rolling counter for the given {@link StatisticsTypeEnum}
   * type.
   * <p>
   * The {@link StatisticsTypeEnum} must be a "counter" type <code>StatisticsTypeEnum.isCounter() ==
   * true</code>.
   *
   * @param type StatisticsTypeEnum defining which counter to retrieve values from
   * @return value from the given {@link StatisticsTypeEnum} counter type
   */
  public long getRollingSum(StatisticsTypeEnum type) {
  ...
  /**
   * Get the value of the latest (current) bucket in the rolling counter for the given {@link
   * StatisticsTypeEnum} type.
   * <p>
   * The {@link StatisticsTypeEnum} must be a "counter" type <code>StatisticsTypeEnum.isCounter() ==
   * true</code>.
   *
   * @param type StatisticsTypeEnum defining which counter to retrieve value from
   * @return value from latest bucket for given {@link StatisticsTypeEnum} counter type
   */
  public long getValueOfLatestBucket(StatisticsTypeEnum type) {
    Bucket lastBucket = getCurrentBucket();
    if (lastBucket == null) {
      return 0;
    }
  ...
  /**
   * Get an array of values for all buckets in the rolling counter for the given {@link
   * StatisticsTypeEnum} type.
   * <p>
   ...
   *
   * @param type StatisticsTypeEnum defining which counter to retrieve values from
   * @return array of values from each of the rolling buckets for given {@link StatisticsTypeEnum}
   * counter type
   */
  public long[] getValues(StatisticsTypeEnum type) {
    Bucket lastBucket = getCurrentBucket();
    if (lastBucket == null) {
      return new long[0];
    }
  ...
  /**
   * Get the max value of values in all buckets for the given {@link StatisticsTypeEnum} type.
   * <p>
   * The {@link StatisticsTypeEnum} must be a "max updater" type <code>StatisticsTypeEnum.isMaxUpdater()
   * == true</code>.
   *
   * @param type StatisticsTypeEnum defining which "max updater" to retrieve values from
   * @return max value for given {@link StatisticsTypeEnum} type during rolling window
   */
  public long getRollingMaxValue(StatisticsTypeEnum type) {
  ...
  /* package for testing */Bucket getCurrentBucket() {
    ...
    if (currentBucket != null
        && currentTime < currentBucket.windowStart + this.bucketSizeInMillseconds) {
        ...
        if (buckets.peekLast() == null) {
          ...
          for (int i = 0; i < numberOfBuckets; i++) {
            ...
            if (currentTime < lastBucket.windowStart + this.bucketSizeInMillseconds) {
              ...
              // we'll just use the latest as long as we're not AFTER the window
              return lastBucket;
            } else if (currentTime - (lastBucket.windowStart + this.bucketSizeInMillseconds)
                > timeInMilliseconds) {
  ...
  /* package */static class Bucket {
    ...
    Bucket(long startTime) {
       ...
       * of all types for each so we only allocate the objects if the StatisticsTypeEnum matches
      ...
      adderForCounterType = new LongAdder[StatisticsTypeEnum.values().length];
      ...
      updaterForCounterType = new LongMaxUpdater[StatisticsTypeEnum.values().length];
      for (StatisticsTypeEnum type : StatisticsTypeEnum.values()) {
    ...
    long get(StatisticsTypeEnum type) {
      if (type.isCounter()) {
    ...
    LongAdder getAdder(StatisticsTypeEnum type) {
      if (!type.isCounter()) {
    ...
    LongMaxUpdater getMaxUpdater(StatisticsTypeEnum type) {
      if (!type.isMaxUpdater()) {
   ...
   * <p>
   * It purposefully does NOT implement Deque or some other Collection interface as it only
   * implements functionality necessary for this RollingNumber use case.
   * <p>
   * Important Thread-Safety Note: This is ONLY thread-safe within the context of RollingNumber and
   * the protection it gives in the <code>getCurrentBucket</code> method. It uses AtomicReference
   * objects to ensure anything done outside of <code>getCurrentBucket</code> is thread-safe, and to
   * ensure visibility of changes across threads (ie. volatility) but the addLast and removeFirst
   * methods are NOT thread-safe for external access they depend upon the lock.tryLock() protection
   * in <code>getCurrentBucket</code> which ensures only a single thread will access them at at
   * time.
   * <p>
   * benjchristensen => This implementation was chosen based on performance testing I did and
   * documented at: http://benjchristensen.com/2011/10/08/atomiccirculararray/
   */
  /* package */static class BucketCircularArray implements Iterable<Bucket> {
    ...
    BucketCircularArray(int size) {
      AtomicReferenceArray<Bucket> _buckets = new AtomicReferenceArray<Bucket>(
          size + 1); // + 1 as extra room for the add/remove;
