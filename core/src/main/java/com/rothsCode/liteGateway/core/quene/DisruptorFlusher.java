...
/**
 * @author roths
 * @Description: 基于disruptor的内存队列
 * @date 2023/8/21 14:22
 */
public class DisruptorFlusher<E> implements Flusher<E> {
  ...
  private DisruptorFlusher(Builder<E> builder) {

    this.executorService = Executors.newFixedThreadPool(builder.threads,
        new ThreadFactoryBuilder()
            .setNameFormat("DisruptorFlusher-" + builder.namePrefix + "-pool-%d").build());
  ...
  public void add(E event) {
    ...
    if (temp == null) {
      process(this.eventListener, new IllegalStateException("DisruptorFlusher is closed"), event);
      return;
    }
    try {
  ...
  public void add(@SuppressWarnings("unchecked") E... events) {
    final RingBuffer<Holder> temp = ringBuffer;
    if (temp == null) {
      process(this.eventListener, new IllegalStateException("DisruptorFlusher is closed"), events);
      return;
    }
    try {
  ...
  public static class Builder<E> {
    ...
    public DisruptorFlusher<E> build() {
      return new DisruptorFlusher<>(this);
    }
  }
  ...
  private class HolderEventFactory implements EventFactory<Holder> {
    ...
    @Override
    public DisruptorFlusher<E>.Holder newInstance() {
      return new Holder();
    }
  }
  ...
  private class HolderWorkHandler implements WorkHandler<Holder> {
    ...
    @Override
    public void onEvent(DisruptorFlusher<E>.Holder event) throws Exception {
  ...
  private class HolderExceptionHandler implements ExceptionHandler<Holder> {
    ...
    public void handleEventException(Throwable ex, long sequence,
        DisruptorFlusher<E>.Holder event) {
  ...
  private class HolderEventTranslator implements EventTranslatorOneArg<Holder, E> {
    ...
    @Override
    public void translateTo(DisruptorFlusher<E>.Holder holder, long sequence, E event) {
